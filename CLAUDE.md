...
├── parser.py   # Protocol parsing, speech extraction, MCP client
...
**Key Classes:**
- `BundestagMCPClient` (parser.py): Async context manager, handles MCP session initialization and tool calls via JSON-RPC over HTTP. Fetches Plenarprotokolle full text and parses speeches by party.
...
- `parse_speeches_from_protocol()` (parser.py): Regex-based parser that extracts individual speeches from protocol text, matching pattern `Speaker Name (PARTY):`
