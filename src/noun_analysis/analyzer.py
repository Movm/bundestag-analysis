...
from .categorizer import CategoryCounts, ToneScores, TopicScores, WordCategorizer
...
class AnalysisResult:
    ...
    all_word_counts: Counter = field(default_factory=Counter)  # All lemmas for extended categories
    total_words: int = 0
    total_nouns: int = 0
    ...
    def to_dict(self) -> dict:
        ...
        if self.topic_scores:
            result["topic_scores"] = self.topic_scores.to_dict()
...
class WordAnalyzer:
    ...
    STOPWORD_NOUNS = {
        ...
        # Parliamentary greeting formulas (often misclassified as nouns)
    ...
    STOPWORD_VERBS = {
        # Auxiliary/modal verbs (including subjunctive forms)
        "sein", "haben", "werden", "können", "müssen", "sollen", "wollen",
        "dürfen", "mögen", "lassen", "möchte", "möchten",
        # Generic verbs
    ...
    # Combined generic stopwords for simple text filtering (used by speaker_export.py)
    # This is a broader set than the type-specific stopwords above
    STOPWORD_GENERIC = {
        ...
        'das', 'der', 'die', 'und', 'ist', 'ich', 'sie', 'wir', 'uns', 'ihnen',
        'meine', 'damen', 'herren', 'kolleginnen', 'kollegen', 'frau',
        'herr', 'präsidentin', 'präsident', 'liebe', 'lieber',
        'geehrte', 'geehrter', 'geehrten', 'verehrte', 'verehrten', 'verehrter',
        'vielen', 'dank', 'danke', 'also', 'gibt', 'möchte', 'möchten',
        # Past participles and verb conjugations (not meaningful as signature words)
        'wurde', 'wurden', 'gesagt', 'gemacht', 'gegeben', 'genommen', 'gesehen',
        'gekommen', 'gegangen', 'gebracht', 'gehalten', 'gestellt', 'gefunden',
        'erhält', 'erhält', 'bleibt', 'liegt', 'steht', 'gilt',
        ...
        # Possessive pronouns (all cases)
        'unsere', 'unserer', 'unseren', 'unserem', 'unseres', 'unser',
        'ihre', 'ihrer', 'ihren', 'ihrem', 'ihres', 'euer', 'eure',
        'meiner', 'meinen', 'meinem', 'meines',
        # Common conjunctions and adverbs
        'sondern', 'sowie', 'doch', 'etwa', 'dabei', 'deshalb', 'daher',
        'darum', 'deswegen', 'dadurch', 'dafür', 'dagegen', 'davon', 'dazu',
        'darauf', 'darüber', 'daran', 'darunter', 'daneben', 'darin', 'damit',
        'natürlich', 'tatsächlich', 'wirklich', 'eigentlich', 'überhaupt',
        'genau', 'gerade', 'eben', 'einmal', 'bereits', 'vielleicht',
        'nämlich', 'insbesondere', 'sogar', 'leider', 'will',
        # Other pronouns
    ...
    def analyze_speeches(
        ...
        for doc in self.nlp.pipe(text_generator(), batch_size=batch_size, n_process=1):
            ...
            for i, token in enumerate(doc):
                ...
                # Track all lemmas for extended categorization (Scheme E)
                if not token.is_space and not token.is_punct and len(lemma) >= 2:
                    result.all_word_counts[lemma] += 1
        ...
        # Pass all_word_counts to enable Scheme E extended categories (incl. discriminatory)
        # Pass noun_counts to enable Scheme F topic categorization
        result.category_counts = self._categorizer.categorize_words(
            result.adjective_counts,
            result.verb_counts,
            result.all_word_counts,
            result.noun_counts,
        )
        result.tone_scores = self._categorizer.calculate_tone_scores(
            result.category_counts
        )
        result.topic_scores = self._categorizer.calculate_topic_scores(
            result.category_counts
        )
